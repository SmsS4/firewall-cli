...
def add_dos_blocking(cli):
    ...
    @click.option("--rate-limit", help="number of allowed requests per second", type=int)
    ...
    @click.option("--protocol", help="protocol of connection. defaults to tcp", type=str, default='tcp')
    def slowloris(port: int, connections_limit: int, protocol: str):
        run(f"iptables -I INPUT -p {protocol} --dport {port} -m connlimit --connlimit-above {connections_limit} --connlimit-mask {connections_limit * 2} -j DROP")

    @cli.command(name="dos-syn-flood")
    @click.option("--port", help="port to limit number of connections", type=int)
    @click.option("--rate-limit", help="number of allowed connections", type=int)
    @click.option("--protocol", help="protocol of connection. defaults to tcp", type=str, default='tcp')
    def syn_flood(port: int, connections_limit: int, protocol: str):
        run(f"iptables -I INPUT -p {protocol} --dport {port} -m connlimit --connlimit-above {connections_limit} --connlimit-mask {connections_limit * 2} -j DROP")
